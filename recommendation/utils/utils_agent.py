...
def goal_only_reward(sequence, n_interaction, scribble_iter, repeat_selection, iou_new, df=None):
    ...
    if df is not None:
        ...
        prev_metric = df[((df.scribble_iter - 1) % 3)
                         == ((scribble_iter - 1) % 3)]
        if len(prev_metric) == 0:
            a = 1
            print('no previous metric')
        ...
        metric = iou_new.mean()
        if prev_metric.size == 1:
            a = prev_metric
            print(a)
        
        if not prev_metric.size > 1:
            reward_done = np.array(0.0001)
...
def select_next_non_empty_frame(gt3D):
    # add this to the test, make sure agent always choose a non-empty frame
    non_zero_slices = (gt3D != 0).any(dim=1).any(dim=2).any(dim=2)
    valid_slices_indices = torch.where(non_zero_slices[0])[0].tolist()
    print(valid_slices_indices)
    next_frame = random.choice(valid_slices_indices)
    return next_frame
...
def recommend_frame(cfg_yl, assess_net, agent, device, n_frame, n_objects, all_F, all_P, new_masks_quality, prev_frames,
                    annotated_frames_list, mask_quality, first_frame, max_nb_interactions, gt3D=None):
    if cfg_yl.setting == 'oracle':
        if cfg_yl.method == 'worst':
            ...
            # make sure the frame exist frames
...
def agent_business(phase, agent, max_nb_interactions, n_interaction, first_scribble, old_masks_metric, new_masks_metric, old_frame, sequence,
    ...
    reward_done = np.array(0)
    if not first_scribble and not phase == 'eval':
