...
# data setting
parser.add_argument('--test_data_path', type=str, default='/var/scratch/jliu5/Medical/Datasets/data/train/tumor/colon')
# interaction setting
parser.add_argument('--phase', type=str, default='baseline')  # TODO: pretrain, train, eval
...
def generate_rewards():
    ...
    # TODO: Now it is the whole test dataset, should be adjusted in the future. 
    # TODO: try different way to generate or evaluate the agent, seperately or unionly
    
    #   下面可以被模块化,暂用的test dataset里的东西
    all_dataset_paths = img_datas
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        for n_interaction_i in range(1, 9*(max_nb_interactions) + 1):  # TODO: 12 can be changed
            ...
            if n_interaction % max_nb_interactions == 1:
                ...
                # original way is selecting random slices no matter it contains objects or not
                # next_frame = random.randint(0, image3D.shape[-1] - 1)
                # 在通道维度上应用 any()，然后再在高度和宽度维度上应用 any()，
            ...
            #     print(f"Error: no mask generated for {img_name}, since there is no points in recommendation slices")
