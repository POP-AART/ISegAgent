...
agent_config = edict({
    'agent': {
        ...
        'pretrain_csv': 'pretrained/pretrain.csv',
        'save_result_dir': 'recommendation/agent_data/pretrained'
    },
    'data': {
        'subset': 'train',
    },
...
def pretrain_agent():
    ...
    reward_csv = agent_config.agent.reward_csv
    pretrain_csv = agent_config.agent.pretrain_csv

    agent.memory_pool.basename_csv = pretrain_csv
    agent.to(device)
    assess_net = None

    save_result_dir = agent_config.agent.save_result_dir
    path_to_reward = os.path.join(save_result_dir, reward_csv)
    print(path_to_reward)
