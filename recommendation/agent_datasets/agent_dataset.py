...
class AgentTrain(torch.utils.data.Dataset):
    def __init__(self,
                 split=None,
                 db_root_dir=None,
...
def load_agent_dataset(cfg, seq_list):
    ...
    dataset = AgentTrain(transform=train_transform,
...
from utils.data_loader import Dataset_Union_ALL, Dataset_Union_ALL_Val, Union_Dataloader
from utils.data_paths import img_datas
from torch.utils.data.distributed import DistributedSampler


def get_dataloaders(img_size, multi_gpu=False, num_workers=2, batch_size=1):
    train_dataset = Dataset_Union_ALL(paths=img_datas, transform=tio.Compose([
        tio.ToCanonical(),
        tio.CropOrPad(mask_name='label', target_shape=(img_size, img_size, img_size)), # crop only object region
