...
# wandb.init(project="medsam3D_union_1", name="first run (without lits)")
# %% set up parser
...
class BaseTrainer:
    def __init__(self, model, dataloaders, args):
        ...
        self.init_checkpoint(join(self.args.work_dir, 'sam_med3d_turbo2.pth'))
        ...
        for step, (image3D, gt3D) in enumerate(tbar):
            ...
            if not self.args.multi_gpu or (self.args.multi_gpu and self.args.rank == 0):
                if step % self.args.accumulation_steps == 0 and step != 0:
                    ...
                    # wandb.log({'train_loss': print_loss, 'train_dice': print_dice})
