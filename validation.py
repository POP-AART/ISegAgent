...
import json
import pickle
from utils.click_method import get_next_click3D_torch_ritm, get_next_click3D_torch_2, random_from_slices
from utils.data_loader import Dataset_Union_ALL_Val
import surface_distance
from surface_distance import metrics

parser = argparse.ArgumentParser()
parser.add_argument('-tdp', '--test_data_path', type=str, default='/var/scratch/jliu5/Medical/Datasets/data/validation/tumor/colon')
parser.add_argument('-vp', '--vis_path', type=str, default='None')
parser.add_argument('-cp', '--checkpoint_path', type=str, default='/var/scratch/jliu5/Medical/Datasets/initmodel/union_train/sam_model_loss_best.pth')
parser.add_argument('--save_name', type=str, default='/var/scratch/jliu5/Medical/SAM-Med3D/union_out_dice.py')
...
def finetune_model_predict3D(img3D, gt3D, spacing, sam_model_tune, device='cuda', click_method='random', num_clicks=10, prev_masks=None, norm_transform=tio.ZNormalization(masking_method=lambda x: x > 0)):
    ...
    low_res_masks = F.interpolate(prev_masks.float(), size=(args.crop_size//4,args.crop_size//4,args.crop_size//4))  # [1,1,32,32,32]
...
if __name__ == "__main__":    
    ...
    elif(args.dim==2):  # 2d medical data
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if(os.path.exists(pred_path)):
            ...
            # 2D prediction
                ...
                seg_mask_list, points, labels, iou_list, dice_list, nsd_list = finetune_model_predict2D(
    ...
    with open('result_history.txt', 'a') as f:
        f.write('Model: {}\n'.format(args.checkpoint_path))
        f.write('Dataset: {}\n'.format(args.test_data_path))
        f.write('num_clicks: {}\n'.format(args.num_clicks))
        f.write('Mean IoU: {}\n'.format(sum(all_iou_list) / len(all_iou_list)))
        f.write('Mean Dice: {}\n'.format(sum(all_dice_list) / len(all_dice_list)))
        f.write('Mean NSD: {}\n'.format(sum(all_nsd_list) / len(all_nsd_list)))
        f.write('---------------------------------\n')

    # compute dice for each organ and each point
    ...
    # compute nsd for each organ and each point
    final_nsd_dict = OrderedDict()
    for k, v in out_nsd_all.items():
        organ = k.split('/')[-4]
        final_nsd_dict[organ] = OrderedDict()
    for k, v in out_nsd_all.items():
        organ = k.split('/')[-4]
        final_nsd_dict[organ][k] = v

    # save name reconfig
    if(args.split_num>1):
        args.save_name = args.save_name.replace('.py', f'_s{args.split_num}i{args.split_idx}.py')

    # save dice results into files
    ...
    # save nsd results into files
    with open('union_out_nsd.json', 'w') as f:
        json.dump(final_nsd_dict, f, indent=4)
