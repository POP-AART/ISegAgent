...
class Dataset_Union_ALL(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        # filter other organs, keep tumor
        if 'colon' in self.image_paths[index]:
            self.target_class = 1
        else:
            self.target_class = 2

        # if sitk_image.GetSpacing() != sitk_label.GetSpacing():
        #     print("image size and label size are different")
        #     print(self.image_paths[index])
        #     print(self.label_paths[index])
        #     sitk_label.SetSpacing(sitk_image.GetSpacing())
        #     sitk.WriteImage(sitk_label, self.label_paths[index])
        ...
        # if sitk_image.GetSpacing() != sitk_label.GetSpacing():
        #     sitk_image.SetSpacing(sitk_label.GetSpacing())

        # if sitk_image.GetSpacing() != sitk_label.GetSpacing():
        #     # Resample the label image to match the spacing of the image
        #     print("image size and label size are different")
        #     print(self.image_paths[index])
        #     print(self.label_paths[index])
        #     sitk_label.SetSpacing(sitk_image.GetSpacing())
        ...
        #     resampler.SetSize(sitk_image.GetSize())
        #     resampler.SetOutputSpacing(sitk_image.GetSpacing())
        #     resampler.SetOutputOrigin(sitk_image.GetOrigin())
        #     resampler.SetOutputDirection(sitk_image.GetDirection())
        #     resampled_label = resampler.Execute(sitk_label)
        #     sitk_label = resampled_label
        ...
        if self.mode == "train" and self.data_type == 'Tr':
            # change it if wanna use training script
            return subject.image.data.clone().detach(), subject.label.data.clone().detach()
            # return subject.image.data.clone().detach(), subject.label.data.clone().detach(), sitk_image.GetSpacing(), self.image_paths[index]
        else:
            return subject.image.data.clone().detach(), subject.label.data.clone().detach(), sitk_image.GetSpacing(), self.image_paths[index]   
