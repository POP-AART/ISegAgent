...
# select most error region for interaction
import edt
def get_next_click3D_torch_ritm(prev_seg, gt_semantic_seg):
    ...
    pred_masks = (prev_seg > mask_threshold)
    true_masks = (gt_semantic_seg > 0)
    fn_masks = torch.logical_and(true_masks, torch.logical_not(pred_masks))  # positive
    fp_masks = torch.logical_and(torch.logical_not(true_masks), pred_masks)  # negative
...
# randomly select positive or negative points
...
def random_from_slices(prev_seg, gt_semantic_seg, slice: int):
    ...
    for i in range(gt_semantic_seg.shape[0]):
        ...
        indices = torch.argwhere(to_point_mask[i])
        points = indices[indices[:, 1] == slice]
        # if points.size(0) == 0:
        #     continue

        # Theoretically, we can select a point randomly, but in order to increase the penalty, we select a point that is very far away, according to the shape
        # Problem: Although negative is set, it cannot be considered as fn or fp,
        if points.size(0) == 0:
            print(f"slice {slice} is empty")
            point = torch.tensor([i, prev_seg.shape[-3], prev_seg.shape[-2], prev_seg.shape[-1]])
            is_positive = False
        else:
            point = points[np.random.randint(len(points))]
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    show_point(batch_points[0][0][0].cpu().numpy(), batch_labels[0][0].cpu().numpy(), ax)
